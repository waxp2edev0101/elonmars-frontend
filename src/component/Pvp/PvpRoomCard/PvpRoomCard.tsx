...
import { changePvpStatus, enterRoom } from "../../../store/pvp/actions";
...
const PvpRoomCard = ({price, status, id}:Props) => {
  ...
  const onEnterRoom = async () => {
    ...
    try{
      ...
      if(result) {
        dispatch(enterRoom(IS_TEST_MODE?"0x144d8FD81a9D7C0102267DD7df76d89017374BaF":id, address, price, (res: any) =>{
          if(res.success){
   ...
   return (
      <>
        <Box className={styles.cardbody}>
          ...
          <Box className={styles.room_card_bg}><img alt="" src="/images/room_card.png"/></Box>

          <Box sx={{height: "260px"}}>
            <Box className={styles.card_container}>
              
              <h2><span>{price}</span> BUSD Room</h2>
              <div>Status : {status === 0 ? "Open":"Closed"}</div>
              <div>
