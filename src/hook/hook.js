...
import { BUSD_CONTRACT_ADDRESS, chainId, PVP_CONTRACT_ADDRESS, TOKEN_CONTRACT_ADDRESS, NFT_CONTRACT_ADDRESS } from "./constants";
...
export const getPvpRoomList = async () => {
    ...
    let result = await pvpContract.methods.getRoomList().call();
...
export const mintNFT = async (address, type) => {
    ...
    console.log(amount)
    await busdContract.methods.approve(NFT_CONTRACT_ADDRESS[chainid], amount).send({
        from: address,
        gas: 270000,
        gasPrice:0
    });
